...
export default class UdpClientPushSensor extends Sensor {
    ...
    format(msg) {
      ...
      if (this.meta.name === 'displacement') {
          ...
          values = {
              "displacement": parseFloat(result[2]).toFixed(2)
          };
      }else if (this.meta.name === 'deflection') {
          result = deflectionReg.exec(msg);
          values = {
              "deflection": parseFloat(result[2]).toFixed(2)
          };
          ...
          if( result[2] == "1" || result[2] == "-1" )
          {
              ...
              values = {
                  "verticality": parseFloat(result[2]).toFixed(2)
              };
          }
      }else if (this.meta.name === 'strain') {
          result = strainReg.exec(msg);
          values = {
              "strain": parseFloat(result[2]).toFixed(2)
          };
      }else if (this.meta.name === 'cableforce') {
          result = cableforceReg.exec(msg);
          values = {
              "cableforce": parseFloat(result[2]).toFixed(0)
          };
      }else if (this.meta.name === 'vibration') {
          result = acceReg.exec(msg);
          values = {
              "x": ((parseFloat(result[2])) * 9.8 * 100 / 16328).toFixed(1),
              "y": ((parseFloat(result[3])) * 9.8 * 100 / 16328).toFixed(1),
              "z": ((parseFloat(result[4])) * 9.8 * 100 / 16328).toFixed(0)
          };
      }else if (this.meta.name === 'temperature and humidity') {
          result = wsdReg.exec(msg);
          values = {
              "temperature": parseFloat(result[2]).toFixed(1),
              "humidity": parseFloat(result[3]).toFixed(1)
          };
      } else if (this.meta.name === 'corrosion') {
          result = corrosionReg.exec(msg);
          values = {
              "corrosion": parseFloat(result[2]).toFixed(2)
          };
      }else if (this.meta.name === 'trafficload') {
          var idStr = (msg.slice(6,38)).toString('utf8');
          var idArray = [];
          ...
          let str = (msg.slice(70,90)).toString('utf8');
          ...
          values = {
              ...
              "acrosstag":parseInt((msg.slice(298,300)).toString(),16),
              "overweighttag": parseInt((msg.slice(300,302)).toString(),16)
          };
      }
