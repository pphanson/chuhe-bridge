    ...
    initMonitor() {
        ...
        net.createServer(function(sock) {
            ...
            sock.on('data', function(data) {
                ...
                console.log(data);
                let idStr = (data.slice(3,19)).toString('utf8');
                let idArray = [];
                for (let i = 0; i < idStr.length; i += 2) {
                    let temp = parseInt(idStr[i] + idStr[i + 1], 16);
                    idArray[i / 2] = temp;
                }
                ...
                values = {
                    ...
                    "year":(data.slice(48,50)).readUInt16BE(),
                    "month":(data.slice(50,51)).readUInt8(),
                    "day":(data.slice(51,52)).readUInt8(),
                    "hour":(data.slice(52,53)).readUInt8(),
                    "minute":(data.slice(53,54)).readUInt8(),
                    "second":(data.slice(54,55)).readUInt8(),
