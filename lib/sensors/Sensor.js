        ...
        for (let key in this.meta.values) {
            if (key) {
                ...
                if (alarmLever > 0)
                {
                    ...
                    let sensorType = JSON.stringify(this.config.id).substr(1, 2);
                    let doc = {
                        sensor_name: this.config.name,
                        timestamp: this._lastUpdatedTime * 1,
                        sensor_id: this.config.id,
                        sensor_type: sensorType,
                        sensor_type_name: this.config.meta._name,
                        alarm_obj: key,
                        alarm_type: '数值报警',
                        alarm_lever: alarmLever,
                        alarm_value: value[key],
                        alarm_content:'数值异常',
                        alarm_position: this.config.postion
                    };
                    let connect = await storage.connection.internalConnection;
