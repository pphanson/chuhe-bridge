...
async function getresult(from, to) {
    ...
    let result = {
        "simpleSensor": simpleSensor.sort(sortNumber).slice(0, 5),
        "sensorType": sensorType.sort(sortNumber).slice(0, 4),
        "sensorPosition": sensorPosition.sort(sortNumber).slice(0, 4)
    };
    return result;
}
