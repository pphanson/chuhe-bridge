...
async function allEvents(from, to, page,keyWords) {             // page总数待优化
    ...
    if (keyWords !== undefined) {            // 参数为4的时候，先获取到特殊事件名字中含有查询条件keywords的事件Id，得到一个数组,然后再放入下一个查询条件
        ...
        count = await collection2.find({$and:[{"startTime": {"$gt": parseFloat(from),
            "$lt": parseFloat(to)}},{$or:[{"eventName": /keyWords/},
            {"eventTypeId":{$in: eventTypes}}]}]}).count();
        if (count < page * 5) {
            page--;
        }
        allEvent = await collection2.find({$and:[{"startTime": {"$gt": parseFloat(from),
            "$lt": parseFloat(to)}},{$or:[{"eventName": /keyWords/},
            {"eventTypeId":{$in: eventTypes}}]}]}).skip(page * 5).limit(15).toArray();
    } else {
        count  = await collection2.find({"startTime": {"$gt": parseFloat(from), "$lt": parseFloat(to)}}).count();
        if (count < page * 5) {
            page--;
        }
        let eventNamesPromise = collection1.find({}).sort({"eventTypeId": 1}).toArray();
        let allEventPromise = collection2.find({"startTime": {"$gt": parseFloat(from), "$lt": parseFloat(to)}}).skip(page * 5).limit(15).toArray();
        eventTypeInfo = await eventNamesPromise;
        allEvent = await allEventPromise;
    }
    for (let i = 0; i < allEvent.length; i++) {         // 给查询的特殊事件添加事件类型名称
