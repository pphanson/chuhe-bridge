...
async function allEvents(from, to, page, keyWords) {             // page总数待优化
    ...
    if (keyWords !== undefined) {                                // 参数为4的时候，先获取到特殊事件名字中含有查询条件keywords的事件Id，得到一个数组,然后再放入下一个查询条件
        ...
        count = await collection2.find({
            $and: [
                {"startTime": {"$gt": parseFloat(new Date(from).getTime()), "$lt": parseFloat(new Date(to).getTime())}},
                {$or: [
        ...
        }).skip(page * 10).limit(10).toArray();
    } else {
        count = await collection2.find({
            "startTime": {
                "$gt": parseFloat(new Date(from).getTime()),
                "$lt": parseFloat(new Date(to).getTime())
            }}).count();
        if (count < page * 5) {
            page--;
        }
