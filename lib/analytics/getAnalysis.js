...
async function getAnalysis(sensor1Id, sensor2Id, from, to) {
    let loc1;                   // 声明两个传感器在获取的传感器对象数组中的位置
    let loc2;
    let result = [];
    let sensors = await getSensors.getSensors();        // 获取全部传感器对象
    ...
    let interval1 = sensors[loc1].meta._monitor.interval;           // 获取传感器间隔时间，数据的Key,异常处理的范围
