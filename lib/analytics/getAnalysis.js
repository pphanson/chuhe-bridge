...
async function getAnalysis(sensor1Id, sensor2Id, from, to) {
    ...
    if (new Date(from) * 1 > new Date(to) * 1) {
        return "开始时间必须小于结束时间";
    }
    let loc1;
    let loc2;
    ...
    let s1 = await sensors[loc1].storage.queryStats(new Date(from), new Date(to));
    let s2 = await sensors[loc2].storage.queryStats(new Date(from), new Date(to));
    let avg1 = parseFloat(s1[Object.keys(s1)[0]].avg.toFixed(2));
    let avg2 = parseFloat(s2[Object.keys(s2)[0]].avg.toFixed(2));
    // 处理异常，并给数据加上时间点
    ...
    b = Lxy / Lxx;                      // 直线的斜率
    a = conY - b * conX;                // 直线的截距
    R = parseFloat((Lxy / (Math.sqrt(Lxx) * Math.sqrt(Lyy))).toFixed(2)); // 相关系数  保留
    if (isNaN(R)) {
        R = 0;
    }
    // R2 = R * R;                                 //判定系数
