...
export default class MongoSensorStorage extends SensorStorage {
    ...
    async _findAndUpdateValue(time, rawValue, sensorid) //查找并更新值
    {
        ...
        if (interval === 60 * 1000) {
            ...
            timeslot = Math.ceil((time.getMinutes() * 60 + time.getSeconds()) / 5);
        } else if (interval === 60 * 60 * 1000) {
            timeslot = 0;
        } else if (interval === 100) {
            timeslot = Math.ceil((time.getMinutes() * 60 * 1000 + time.getSeconds()*1000 + time.getMilliseconds()) / 100);
        }
    ...
    _createBlankValues() {
        ...
        const interval = this.sensor.config.meta._monitor.interval;//config.monitor.interval

        if (interval === 60 * 1000 || interval === 5000 || interval === 60 * 60 * 1000 || interval === 100) {
            const values = [];
